...
import { Betaling, Trekking, Resultaat, User, Rol } from '@/lib/types';

// "Nog X dagen"-badge op het dashboardkaartje — 0 op zaterdag zelf.
function dagenTotVolgendeTrekking(): number {
  const dag = new Date().getDay();
  return dag === 6 ? 0 : 6 - dag;
}

// Gedeelde "tegel-als-knop"-stijl — lichte bevel-rand + zachte
// schaduw, zodat een tegel er tastbaar/klikbaar uitziet zonder een
// los "Alle →"-tekstje nodig te hebben.
...
// Winnaar scherm — toont resultaat.prijsBedrag (vastgelegd server-side
// op het moment van winnen), nooit een live herberekend bedrag.
function WinnaarScherm({ resultaat, trekking, kashouder, onGeclaimd }: { resultaat: Resultaat; trekking: Trekking; kashouder: User | null; onGeclaimd: () => void }) {
...
interface SpelerDashboardProps {
  /** Welke rol(len) deze route bedient — de aanroepende pagina regelt
   *  zelf de redirect voor andere rollen; dit is alleen de spinner-gate
   *  om te voorkomen dat verkeerde inhoud even opflitst voordat die
   *  redirect is uitgevoerd. */
  allowedRoles: Rol[];
  /** Extra kaart(en), bijv. de beheerder-only "Vereist aandacht"-tegel —
   *  wordt gerenderd direct na de wachtrij-banner, vóór de prijzenpot. */
  extraTop?: ReactNode;
}

export function SpelerDashboardContent({ allowedRoles, extraTop }: SpelerDashboardProps) {
  ...
  // BUGFIX: stond eerder ná een voorwaardelijke return (zie onder) —
  // een hook die soms wel, soms niet wordt aangeroepen is een harde
  // schending van de Hooks-regels en kan de pagina laten crashen
  // zodra profileLoading van true naar false wisselt. Hoort hier,
  // samen met de andere useState-aanroepen, vóór elke early return.
  const [winnaarGeclaimd, setWinnaarGeclaimd] = useState(false);
  ...
  if (profileLoading || (profile && !allowedRoles.includes(profile.rol))) {
    return (
  ...
  return (
    <>
      ...
      <div className="page">
        ...
        {/* Prijzenpot — bewust GEEN link (verwees eerder naar /kas —
            dat scherm is voor de financiën, niet bedoeld voor leden) */}
      ...
      <nav className="bottom-nav">
        {[
          { href: profile?.rol === 'beheerder' ? '/beheerder' : '/dashboard', icon: '🏠', label: 'Dashboard' },
